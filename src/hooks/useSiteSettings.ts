  ...
  const fetchSiteSettings = async () => {
    try {
      ...
      const settings: SiteSettings = {
        ...
        address: data.find(s => s.id === 'address')?.value || 'Naga City, Camarines Sur',
        ...
        messenger_id: data.find(s => s.id === 'messenger_id')?.value || '100064173395989',
        contact_phone: data.find(s => s.id === 'contact_phone')?.value || '09569414260',
        contact_email: data.find(s => s.id === 'contact_email')?.value || 'support@daplash.com',
        other_menu_link: data.find(s => s.id === 'other_menu_link')?.value || '',
        starting_point_lat: parseFloat(data.find(s => s.id === 'starting_point_lat')?.value || '13.6218'),
        starting_point_lng: parseFloat(data.find(s => s.id === 'starting_point_lng')?.value || '123.1948'),
