                ...
                <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
                    <div className="w-full lg:w-1/3">
                        <div className="lg:sticky lg:top-32 text-center lg:text-left">
                            <div className="w-14 h-14 sm:w-16 sm:h-16 bg-blue-100 rounded-2xl flex items-center justify-center text-daplash-blue mb-6 mx-auto lg:mx-0">
                                <HelpCircle size={32} />
                            </div>
                            <h2 className="text-3xl sm:text-4xl font-black text-daplash-dark mb-4 sm:mb-6">
                                ANY <span className="text-daplash-blue">QUESTIONS?</span>
                            </h2>
                            <p className="text-base sm:text-lg text-gray-600 mb-6 sm:mb-8">
                                Find answers to the most common questions about our services.
                            </p>
                            <a href="https://m.me/100064173395989" target="_blank" rel="noopener noreferrer" className="text-daplash-blue font-bold flex items-center justify-center lg:justify-start space-x-2 hover:underline">
                    ...
                    <div className="w-full lg:w-2/3 space-y-3 sm:space-y-4">
                        {faqs.map((faq, i) => (
                            <div
                                key={i}
                                className={`border-2 rounded-2xl transition-all duration-300 ${activeIndex === i ? 'border-daplash-blue bg-blue-50/30' : 'border-gray-100 bg-white'}`}
                            >
                                <button
                                    onClick={() => setActiveIndex(activeIndex === i ? null : i)}
                                    className="w-full px-5 sm:px-8 py-4 sm:py-6 flex items-center justify-between text-left"
                                >
                                    <span className={`text-base sm:text-xl font-bold leading-snug ${activeIndex === i ? 'text-daplash-blue' : 'text-daplash-dark'}`}>
                                        {faq.q}
                                    </span>
                                    <div className={`shrink-0 ml-4 transition-transform ${activeIndex === i ? 'rotate-180' : ''}`}>
                                        {activeIndex === i ? <Minus className="text-daplash-blue" /> : <Plus className="text-gray-400" />}
                                    </div>
                                </button>
                                <AnimatePresence>
                                    {activeIndex === i && (
